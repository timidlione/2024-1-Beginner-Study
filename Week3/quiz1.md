...
- 답: GDSC: Google Developer Student Clubs

### Q1
Git에서 파일의 상태는 크게 untracked와 tracked로 나눌 수 있다.  
그렇다면 tracked에는 어떤 상태가 있는지 모두 적으시오.

- 답: 세 가지 영역: Working Directory, Staging Area, Repository
...
- 답: develop 브랜치를 생성하고 이동: git checkout -b develop

### Q4
Working Directory에 있는 파일들을 모두 Staging Area에 추가할 수 있는 명령어를 적으시오.

- 답: 모든 Working Directory 파일을 Staging Area에 추가: git add .
...
Create a merge commit: A, B, C의 커밋을 그대로 병합
Squash and merge: 여러 개의 커밋을 하나로 합침
Rebase and merge: 최신 commit을 base로 변경
...
- 답: 현재 작업 중인 branch는 develop branch이고 readme 파일을 추가하는 commit이 생성 되었고 해당 commit의 id는 a1s2d3f이다.
...
- 답: Reset: git reset --soft 345hj26
...
- 답: Revert: git revert a1s2d3f

### Q9
여러 사람이 협업하는 프로젝트에서 커밋을 되돌려야 한다.  
reset과 revert 중에 어떤 것을 선택할 것인지를 그 이유와 함께 적으시오.

- 답: 선택: revert, 다른 사람이 작업 중인 브랜치에서 reset을 사용할 경우 다른 사람이 올린 commit이 삭제될 수 있음.